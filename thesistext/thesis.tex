...
% ---------------------------
% Example listing refs you can adapt:
% \lstinputlisting[language=CMake,caption={CMake configuration.},label={lst:cmake}]{CMakeLists.txt}
% \lstinputlisting[language=Docker,caption={Dockerfile for the service.},label={lst:dockerfile}]{Dockerfile}
% \lstinputlisting[language=C++,caption={Server entrypoint with route registration.},label={lst:server-main}]{src/main.cpp}
% \lstinputlisting[language=C++,caption={HTTP handler with dispatcher and debug modes.},label={lst:http-handler}]{src/http/http_handler.cpp}
% \lstinputlisting[language=C++,caption={JSON validation helpers.},label={lst:json-validate}]{src/util/json_debug.hpp}
% \lstinputlisting[language=C++,caption={Enriched OSRM model (subset).},label={lst:osrm-model}]{src/models/osrm_enriched.hpp}
% \lstinputlisting[language=C++,caption={Selected from\_json converters (defensive).},label={lst:osrm-fromjson}]{src/models/osrm_enriched.hpp}
