...
\subsubsection{Supplementary Techniques for Segmentation and Scoring}
...
\paragraph{Shortest arc and distance calculations.}  Calculating distances and headings along the earth's surface uses great-circle geometry.
...
\paragraph{Wavelets for change detection.}  Haar wavelets provide a simple differencing operator: applying the wavelet transform to a
...
\paragraph{Hysteresis thresholding.}  Binary decisions based on noisy signals can flicker when a value hovers near a threshold.  Hysteresis
...
\paragraph{Root mean square (RMS) metrics.}  RMS values summarise the effective magnitude of a varying signal by squaring, averaging and taking
...
\paragraph{Gradient variation.}  Changes in road grade constrain a rider's ability to maintain constant power: sustained climbs require higher
...
\paragraph{Curvature and twistiness.}  The curvature of a planar curve is the reciprocal of the radius of its osculating circle.  For polyline
...
\paragraph{Speed and acceleration smoothing.}  Differentiating position to obtain speed and acceleration amplifies high-frequency GPS noise.
...
\paragraph{Segmenting by OpenStreetMap ways.}  OpenStreetMap represents roads as "ways", sequences of nodes with uniform attributes.
