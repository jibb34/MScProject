...
Intuitively, I could segment a route by the roads that it follows. When a route changes to a new road, the route can be segmented. However this can
...
to define a split so that segments occur at meaningful points, and how do I define meaningful?
...
OSRM's \texttt{/match} service implements the HMM paradigm, returning one or more matched sub-traces with per-point confidence \citeyearpar{OSRMMatchAPI}.
...
surface penalties, or junction costs before (re)extracting the graph used by the matcher \citeyearpar{OSRMDocker,OSRMApiProfile,OSRMProfileLua}.
...
fitness websites (Strava, Garmin, MapMyRide, etc.), or directly downloaded from a GPS device. Either way, there are standards to take
...
\begin{table}[ht]
  ...
  \begin{tabular}{l|c|c|c|p{4cm}}
    ...
    Method C (Wavelet, ours) & 13 & 14 & 10 & Balances length and number; some late splits on gradual curves. \\
    \hline
  \end{tabular}
\end{table}
...
Below is the high-level timeline for the project, showing completed milestones in grey and remaining tasks in bold.  Milestones that have passed are either shaded or struck through - please remove lines that no longer apply.
\begin{itemize}
	\item \sout{Week 1--2: Finalise research questions \& write proposal}
	\item \sout{Week 3: Literature review complete; identify core segmentation methods}
	\item \sout{Week 4: Set up OSRM + GPX ingestion prototype}
	\item \sout{Week 5: Implement C++ segmentation engine (curvature \& grade)}
	\item \sout{Week 6: Integrate wavelet \& hysteresis approaches; initial tests}
	\item \sout{Week 7: Develop Training Suitability Scoring engine; tune thresholds}
	\item \sout{Week 8: Web UI prototype for upload and visualization}
	\item \sout{Week 9: Bulk testing on Strava dataset \& performance tuning}
...
The persistence layer stores canonical segment definitions and their occurrences on concrete routes. The core schema comprises three tables:
