...
sources.  The implemented system successfully ingests GPX tracks, performs map matching against OpenStreetMap, enriches tracks with contextual
metadata and segments routes using a reproducible algorithm.  Evaluation on a diverse set of rides showed that the segmentation engine agrees
with human judgement on most meaningful transitions, via similarity scores with Strava. 
