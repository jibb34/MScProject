...
Surface penalties, or junction costs can be adjusted before (re)extracting the graph used by the matcher \citeyearpar{OSRMDocker,OSRMMatchAPI,OSRMProfileLua}.
...
original licences are included in the repository, fulfilling our obligations under the MIT and BSD licences. In practice the OSRM images used for map matching are built from specific OpenStreetMap snapshots, and the pipeline bundles the required Open Database License notice so that downstream users remain aware of the dataset's provenance. Derived routing graphs are recreated on demand rather than redistributed, preserving the intent of the ODbL. Uploaded GPX traces are processed in memory with hashed identifiers and deleted once aggregate statistics have been stored, reducing the surface for data leaks.
\subsection{Social}
...
Reliance on volunteer-curated OpenStreetMap data means that areas with sparse coverage may yield poorer segment suggestions, a bias that communities should be aware of when interpreting results.
\subsection{Ethical}
...
All uploads reside in encrypted temporary storage and only anonymised aggregates leave the processing pipeline, ensuring that individual trajectories cannot be reconstructed by third parties.
...
collective effort to democratise geographic information.
By choosing OSRM, an open-source router built on this data, the project demonstrates that high-quality navigation services can be delivered without relying on proprietary mapping platforms.
By building upon this commons I contribute to the public good rather than locking data behind commercial terms.
However, routing and training tools can influence cycling patterns and therefore intersect with transport policy: highlighting certain
...
This section details the conceptual modules that comprise the Training Suitability System.
\label{sec:moduledesign}
\subsection{GPX-to-Enriched-JSON Pipeline Modules}
The processing layer is decomposed into small, focused modules so that each step can be reasoned about and tested in isolation.
Figure~\ref{fig:moduledeps} gives the high level dependency chain.

\subsubsection{Module Dependency Diagram}
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[node distance=2.5cm and 2.5cm, >=Stealth]
    % Top row
    \node[entity] (reader) {GPX Reader};
    \node[entity, right=of reader] (matcher) {Map Matcher};
    \node[entity, right=of matcher] (enricher) {Enricher};

    % Bottom row
    \node[entity, below=of reader] (segmenter) {Segmentation Engine};
    \node[entity, right=of segmenter] (tss) {TSS Engine};
    \node[entity, right=of tss] (db) {DB Adapter};

    % Arrows top row
    \draw[relation] (reader) -- (matcher);
    \draw[relation] (matcher) -- (enricher);

    % Wrap down
    \draw[relation] (enricher.south) -- ++(0,-0.8) -- (segmenter.north);

    % Bottom row arrows
    \draw[relation] (segmenter) -- (tss);
    \draw[relation] (tss) -- (db);
  \end{tikzpicture}
  \caption{Module dependency diagram for the processing pipeline}
...
\midrule
GPX Reader & GPX file & waypoint list & Parse GPX XML and normalise track points \\
Map Matcher & waypoint list & matched geometry & Aligns GPS track to OSM network via OSRM \\
Enricher & matched geometry & enriched GeoJSON & Augments geometry with OSM metadata and sensor readings \\
Segmentation Engine & enriched GeoJSON & segment list & Detects homogeneous route segments \\
TSS Engine & segment list, activity data & scored segments & Computes Training Suitability Score per segment \\
DB Adapter & scored segments & persistence records & Persists segment definitions and statistics \\
\bottomrule
\end{longtable}

\paragraph{Module Summaries}
\begin{itemize}
  \item \textbf{GPX Reader} normalises timestamps and coordinates into an internal list of points.
  \item \textbf{Map Matcher} uses OSRM to snap the points to the OpenStreetMap network and returns a cleaned geometry.
  \item \textbf{Enricher} fetches contextual attributes for each way (class, speed limit, junction density).
  \item \textbf{Segmentation Engine} implements the two-pass algorithm that groups points into contiguous segments.
  \item \textbf{TSS Engine} analyses rider power and route context to derive per-zone suitability scores.
  \item \textbf{DB Adapter} converts segments into Data Transfer Objects and writes them to the SQL schema.
\end{itemize}
...
The persistence layer is captured by the ERD in Figure~\ref{fig:db_erd}. It stores canonical segment definitions and their
occurrences on uploaded activities.
...
\midrule
segment\_defs & seg\_id (TEXT) & 32-byte SHA-256 hex fingerprint \\
segment\_defs & length (REAL) & segment length in metres \\
segment\_defs & geom (BLOB) & encoded polyline geometry \\
route\_segments & route\_id (INTEGER) & FK to uploaded route/activity \\
route\_segments & seg\_id (TEXT) & FK to segment\_defs.seg\_id \\
route\_segments & start\_idx (INTEGER) & inclusive index of first point \\
route\_segments & end\_idx (INTEGER) & exclusive index after last point \\
segment\_way\_runs & seg\_id (TEXT) & FK to segment\_defs.seg\_id \\
segment\_way\_runs & seq (INTEGER) & order within segment \\
segment\_way\_runs & way\_id (INTEGER) & OSM way identifier \\
\bottomrule
\end{longtable}
...
\midrule
seg\_id & 64 hex chars & Unique SHA-256 identifier for a directional segment \\
route\_id & integer & Internal ID for a matched activity or route \\
start\_idx / end\_idx & integer & Indices into original point array (end exclusive) \\
geom & encoded polyline & Compressed segment geometry using Google polyline encoding \\
way\_id & integer & OpenStreetMap way identifier forming part of a segment \\
tss\_z1 -- tss\_z5 & float & Per-zone training suitability scores (0--1) \\
hit\_count & integer & Number of times segment has been observed \\
\bottomrule
\end{longtable}
...
rolling countryside routes and mountainous climbs.  Processing time was measured at approximately 12~s per hour of ride on a consumer laptop (Lenovo Thinkpad
L480) when parallelising OSRM queries, and appoximately 5 seconds per hour on a more powerful AMD Ryzen 7 processor.

Segmentation was determined by both qualitative and quantitive methods, relating to two popular segment-based alternatives: Strava and Komoot. This is discussed
in the next section.

While not fully implemented, Training Suitability Scoring can be tested in the field. By running the program with input from a cycling team in a training camp, 
it should be possible to quickly gather enough data to see trends in the scoring. After enough data, it can be validated by having independent cyclists from people used
to train the data perform a targeted training session on routes highlighted by the TSS algorithm, and then they will have their workout compliance measured by their coaches.

\section{Comparison of Segmentation with Existing Segment Creation Tools}
...
By running 20 separate GPX files through the segmentation engine, and getting the start and end positions of their segments (as well as the total
segment length), we were able to calculate a simliarity score via the following formula:
\[
\mathrm{Sim}(S,E) = \exp\!\left(-\frac{d(s_{\mathrm{start}}, e_{\mathrm{start}}) + d(s_{\mathrm{end}}, e_{\mathrm{end}})}{2 \, \tau}\right),
\]
\paragraph{Similarity Metric:}
We define similarity between a Strava segment $S$ and an engine segment $E$ by comparing their start and end coordinates. Let $d(p,q)$ denote the haversine distance (in meters) between points $p$ and $q$ on the Earth’s surface. If $S = (s_{\mathrm{start}}, s_{\mathrm{end}})$ and $E = (e_{\mathrm{start}}, e_{\mathrm{end}})$, then the similarity score is
where $\tau$ is a scaling constant (e.g., $100$ m) that determines how quickly similarity decays with distance. The score lies in the range $[0,10]$, with values close to $10$ indicating near-identical terminuses.
We use terminal points rather than overall route matching here to save time mostly, and due to the nature of segments existing on a road network, more often than not if two teriminuses are the same, then the route in between will be the same as well.


By using the Strava API to get the segments that ran along the routes that I took the GPX files from, we are able to get a significant amount of data to compare, and putting the results on a logarithmic scale
we get some interesting results, as seen in \ref{fig:compare}.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{graph_compare.png}
  \label{fig:compare}
  \caption{A clear correlation between strava and segmentation engine results.}
\end{figure}
There is a clear correlation, especially between the 400m and 1.2km range (where segment popularity is the highest), and it forms a normal-ish distribution. It does seem skewed right a bit, as well
as there being a distinct drop at 400m. This is to be expected though, as the minimum segment length I set for the test was 400m. Anything less than this was merged into another segment.
However, it seems that the ideal segment length is around the 800 metre range. So optimising my algorithm around that length makes the most sense.

\section{Final Thoughts}

\chapter{Results and Conclusions}
\label{chap:results}
This project set out to determine whether road suitability for structured cycling training could be quantified using only ride data and open mapping sources.
The implemented system successfully ingests GPX tracks, performs map matching against OpenStreetMap via OSRM, enriches tracks with contextual metadata and segments routes using a reproducible algorithm.
Evaluation on a diverse set of twenty rides cross-referenced with Strava segments showed that the segmentation engine agrees with human judgement on most meaningful transitions.
In preprocessing trials, a Kalman smoother reduced positional jitter by roughly 12\% compared to a simple hysteresis filter, yielding more stable boundaries.
Building on OSRM and the OpenStreetMap dataset proved sufficient for high-quality routing without resorting to proprietary services. The pipeline trims the first kilometre of every upload, hashes rider identifiers and discards raw files after aggregation, demonstrating that useful segment analytics can be produced while respecting data privacy.

The main contribution of this work is the introduction of an individual-agnostic segmentation algorithm that relates specifically to training
suitability. While the context of this work we focused on elevation gain, the framework has space and functionality for utilising other metrics,
such as heading delta, OpenStreetMap way category, and feature segmentation. By categorising elevation into four distinct sections, this allows
a start for implmenting Training Suitability scoring, since gradient variation of a road is a huge factor in cyclists not achieving their
power targets for a given workout. This framework hopefully will also allow others to build upon it, and with collective knowledge, implement
a robust method for creating a score system for the training segments. 
The results of the work also highlight limitations; the feature-based segmentation model and scoring model is not sufficient for my goal, since
it turns out that the OpenStreetMap has no unified standard for what features to include. The conclusion is therefore cautiously optimistic: quantifying training suitability is feasible and valuable, but further
refinement and community validation are required for widespread adoption.

\section*{Reflection}
Developing the system highlighted the trade-offs between leveraging rich open datasets and safeguarding the privacy of riders who contribute their traces. Iterating between Kalman smoothing and hysteresis filters emphasised how algorithmic choices shape both accuracy and user trust. Future iterations should continue to question data retention practices and to document the provenance of the open tools on which the platform depends.

\chapter{Further Discussions and Research Gaps}
\label{chap:discussion}
Several avenues exist for future improvement.  First, the segmentation algorithm currently operates with fixed thresholds. Experimenting with
...
Training Suitability Score could be improved upon by incorporating additional data sources such as live traffic volumes, air quality indices and weather
...
Any other time-series or distance-series data from routes could be taken and implemented into the algorithm, as the main functionality of this project is
discrete signal analysis at the end of it all.
...
A key design decision was to keep the wavelet-based change detection and resampling routines agnostic to the type and number of input signals. This opens the 
door to refining segmentation by fusing heterogeneous variables—angular measures such as heading or gradient changes, categorical descriptors 
like surface type or junction class, and continuous metrics—within a unified multi-signal framework.

Some other future research could maybe be done into the reason why 800m seems like the most common segment length, and if this has anything to do with
training suitability, or if it is just a product of normal distribution of road lengths.
...
\label{apx:glossary}
\begin{description}
  \item[GPX] GPS Exchange Format used to encode routes and activities.
  \item[GeoJSON] JSON-based format for geographic data structures.
  \item[OSM] OpenStreetMap, the open collaborative map database.
  \item[OSRM] Open Source Routing Machine providing map matching services.
  \item[TSS] Training Suitability Score assigned to each segment.
  \item[ERD] Entity--Relationship Diagram illustrating database structure.
  \item[DTO] Data Transfer Object used for moving data between modules.
\end{description}
