...
\graphicspath{{assets/}}
\usepackage{listings}
\lstset{breaklines=true, columns=fullflexible, keepspaces=true}
...
A useful way to clean up the location data as a preprocessing step could be through map matching. Newson and Krumm \citeyearpar{NewsonKrumm2009} explore
...
This article by Indoor Cycling Association \citeyearpar{indoorcycling_redzone} explains the 7-training-zone model; a good starting point for cutoff times for tuning Wavelet Footprints
...
\label{sec:specific}
\paragraph{User Interface Requirements:} Description of UI requirements.\\
...
\end{enumerate}
\paragraph{Software Interface Requirements:} APIs and protocols.\\
...
\begin{table}[h!]
        ...
        \begin{tabular}{|l|p{5cm}|l|l|p{4cm}|}
                \hline
                \textbf{Requirement ID} & \textbf{Description} & \textbf{Type} & \textbf{Priority} & \textbf{Source} \\
                \hline
                DBR-01 & Store segment definitions with unique identifiers & Data & High & Design \\
                DBR-02 & Record Training Stress Scores for each segment occurrence & Data & Medium & Analysis \\
                \hline
        \end{tabular}
...
as "ignore" per section. The multi-chunk approach to processing also allows for error, without corrupting the entire data set. If a chunk is corrupted,
...
\paragraph{Security}
Security isn't as important for this proof of concept, but a lot of standard considerations can be made that relate to LAMP stack web applications:
\begin{itemize}
	\item All external HTTP endpoints \emph{must} require HTTPS/TLS.
...
Each piece of software used (detailed in Operating Environment) should be kept up to date for security features, but should be versioned so that
if an update breaks the software, it can be rolled back.

\paragraph{Portability}
The whole project will be made available via GitHub, and possibly a fully dockerized version of the system will be developed in the future for portability.
Since the program runs on a specific flavour of Linux (arch), the environment must be fully replicated at the moment in order to work. Dockerising
should solve this (although since we use Docker already, we would need to create a Docker network to communicate between components).
...
\begin{itemize}
	...
	\item Week 9: Bulk testing on Strava data set \& performance tuning
...
The overview of the system is presented in Figure \ref{fig:archdiagram}. It shows the major components of the system and how they will interact.
...
The function of the system is enabled by the following sub-components:
\begin{table}[H]
        \centering\small
        \caption{Architectural Views - structured summary}
...
Docstrings for clarity, and the Google C++ style for the engine code.  Continuous integration scripts enforce linting and testing was done after each module implementation.

\section{Module-Level Design}
This section details the conceptual modules that comprise the Training Suitability System. This is different to components; the components
enable the functionality of the modules.
...
segments and scores.  For the pipeline, a large GPX library verified that GPX parsing, OSM fetching and enrichment functions handled edge cases such as missing
extensions or malformed XML.  Integration tests exercised the entire pipeline on a set of twenty rides covering flat urban loops,
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{l X X}
        ...
        \midrule
        How pieces are made & Riders draw their own; can be noisy or gamed & Cut up by rules (wavelets + RMS), deterministic \\
        Where pieces show up & Many in busy areas, fewer in quiet ones & Works anywhere a ride goes \\
        What each piece shows & Leaderboards; time/speed only & Adds slope, power, heart rate, effort \\
        Clear or hidden & Rules are hidden; start/end unclear to users & Transparent, rule-driven; can be explained \\
        \bottomrule
    \end{tabularx}
\end{table}
