...
/** Call /lab/resample for the wavelet terrain and overlay it on the elevation plot. */
  ...
  async function runWaveletTerrain() {
    ...
    const payload = { map: sel, fn: "terrain", params: { terrain: p }, persist };
  ...
  // ===== Draw green DB segment matches =====
  function overlayDbMatches(data) {
    const show = document.getElementById('toggle-db-matches')?.checked ?? true;
    if (!show) return;

    const segs = data.existing_segments || [];
    ...
    const xScale = window.graphXScale || ((i) => i);
    const yTop = 10;
    const bandH = 6;
    ...
    segs.forEach(s => {
      const x1 = xScale(s.start_idx);
      const x2 = xScale(s.end_idx);
      ctx.fillRect(Math.min(x1, x2), yTop, Math.abs(x2 - x1), bandH);
    });
    ctx.restore();
  }
...
fetch('/dbping')
  .then(r => {
    console.log('[dbping] raw response:', r.status, r.headers.get('content-type'));
    return r.text();
  })
  .then(txt => {
