(function(){
  ...
  // colour ramp helpers (for gradient-coloured fill)
  ...
  const stops=[{g:-30,c:'#0077b4'},{g:0,c:'#3f77b4'},{g:3,c:'#2ca02c'},{g:8,c:'#ff7f0e'},{g:10,c:'#d62728'},{g:15,c:'#d62728'},{g:15,c:'#000000'}];
  ...
  function colorForGrad(g){
    if (!Number.isFinite(g)) return '#888'; // neutral gray for bad values
    for (let i=0;i<stops.length-1;i++){
      const a=stops[i], b=stops[i+1];
      if (g<=a.g) return a.c;
      if (g<b.g)  return lerpColor(a.c,b.c,(g-a.g)/(b.g-a.g));
  ...
  const speed_rs    = V.speed_rs    || [];         // m/s or km/h (auto-fixed)
  const speed_u_skm = V.speed_u_skm || [];         // km on X (uniform dist)
  const speed_u_vals= V.speed_u_vals|| [];         // values (m/s or km/h, auto-fixed)
  ...
  if (mapEl && window.L) {
    ...
    function addPolyline(latlon, opts){ return L.polyline(latlon, Object.assign({weight:3, opacity:0.9}, opts||{})).addTo(map); }

    let bounds = null;

    if (coords_osrm.length>1) {
      const osrmLine = addPolyline(coords_osrm, {color:'#d33'}); // red
      bounds = osrmLine.getBounds();
    }

    if (coords_rs.length>1) {
      // base RS line (blue)
      const rsBase = addPolyline(coords_rs, {color:'#38f', weight:3, opacity:0.9});
      bounds = bounds ? bounds.extend(rsBase.getBounds()) : rsBase.getBounds();

      // per-way segments (if provided)
      if (Array.isArray(way_rs) && way_rs.length) {
        ...
        for (const seg of way_rs) {
          ...
          const col = (typeof seg.color==='string' && seg.color) ? seg.color : '#888';
          const pl = addPolyline(coords_rs.slice(i, j+1), { color: col, weight: 4, opacity: 0.9 });
  ...
  if (!(coords_rs.length>1 &&
    ...
    if (el) el.innerHTML = '<div style="padding:12px;color:#c00">Missing or mismatched arrays (coords/elev/grad/heading).</div>';
    return;
  }
  ...
  // ===== Heading (wrapped to -π..π) - data kept, plot removed =====
  ...
  // ===== Elevation with continuous coloured fill by gradient =====
  ...
  // Smooth gradient a bit (simple windowed average in distance domain)
  // Detect if grad_rs is fraction (<=1) or already percent (>1)
  // This avoids blowing up values and turning most segments black.
  //NOTE: if data changes later, this auto-detection keeps things sane.
  const absMaxGrad = grad_rs.reduce((m,v)=> Number.isFinite(v) ? Math.max(m, Math.abs(v)) : m, 0);
  const grad_pct   = (absMaxGrad <= 1.0) ? grad_rs.map(g => g * 100) : grad_rs.slice();
  ...
  // --- Adaptive segment length helper (shorter when |gradient| is large) ---
  const SEG_LEN_MIN = 12;   // meters (steep)
  const SEG_LEN_MAX = 40;   // meters (flat)
  const G1          = 10;   // % where we start shrinking aggressively
  function segLenForGrad(g) {
    if (!Number.isFinite(g)) return (SEG_LEN_MIN + SEG_LEN_MAX) * 0.5;
    const t = Math.max(0, Math.min(1, Math.abs(g) / G1));  // 0..1
    return SEG_LEN_MAX - (SEG_LEN_MAX - SEG_LEN_MIN) * t;  // meters
  }

  // build coloured fill segments using a constant baseline and 'tonexty'
  let segStart = 0;
  const traces = [];
  while (segStart < n - 1) {
    ...
    const ybase = new Array(xseg.length).fill(eMin - pad); // baseline just below min
    ...
    // Invisible baseline to close fill
    traces.push({
      x: xseg,
      y: ybase,
      mode: 'lines',
  ...
  // --- Overlay: raw elevation outline (thin) ---
  traces.push({
    x: dist_km, y: elev_rs, mode: 'lines', line: { width: 1.2, color: '#333' }, name: 'elev (outline)',
    hovertemplate: 'd=%{x:.3f} km<br>elev=%{y:.1f} m<extra></extra>'
  });

  // --- Overlay: instantaneous speed (scaled) on y2 (if available) ---
  function ensureKmH(arr){ return (arr||[]).map(v => Number.isFinite(v) ? (v>30 ? v : v*3.6) : v); } // best-effort m/s→km/h
  ...
  // ===== Separate speed chart - removed =====
  // (Plotly.newPlot('plotSpeed', ...) was here)

  // ===== Hover pin synced to charts =====
  ...
  function latLngAtDistance(d) {
    ...
    while (lo + 1 < hi) { const mid = (lo + hi) >> 1; if (dist_m[mid] <= d) lo = mid; else hi = mid; }
  ...
  function wireHover(divId){
    ...
    el.on('plotly_hover', (ev) => {
      ...
      showMarker(latLngAtDistance(xkm * 1000.0));
    });
  ...
  // removed: wireHover('plotHeading')
  wireHover('plot');
  // removed: wireHover('plotSpeed')

  // ===== Diagnostics (steps / spacing) =====
  function stepStats(coords){
    if (!Array.isArray(coords) || coords.length < 2) return null;
    const steps = [];
    for (let i=1;i<coords.length;i++) steps.push(haversine(coords[i-1], coords[i]));
