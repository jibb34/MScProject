// seg3d.js — 3D terrain mini-map for Segments tab
(function () {
  'use strict';

  let map3d = null;
  // Skin settings: tune to taste
  const SKIN_HALF_WIDTH_M = 35; // corridor half-width in meters (35 => ~70 m wide)
  const SKIN_STRIDE = 1;        // use >1 to decimate if you have very dense coords

  function init3DMap() {
  if (map3d) return; // already initialised — don't touch the DOM
  const el = document.getElementById('seg-map');
  if (el) el.replaceChildren(); // clear only once, before first Mapbox init
    ...
    map3d.on('load', () => {
      // Use current terrain DEM source (older terrain-rgb URLs 404 for some plans)
      map3d.addSource('mapbox-dem', {
        ...
        // hillshade on top of terrain for topo “feel”
        // insert below first symbol layer so labels stay above
      ...
      //
      // Find the first label layer so we can insert our basemap under labels
      let labelLayerId = null;
      for (const l of map3d.getStyle().layers) {
        if (l.type === 'symbol' && l.layout && l.layout['text-field']) { labelLayerId = l.id; break; }
      }

      // --- OpenTopoMap raster (colored topo) ---
...
// Add the raster layer *on top* so it’s guaranteed to be visible
if (!map3d.getLayer('opentopo-layer')) {
  map3d.addLayer({
    ...
    source: 'opentopo',
    paint: {
      'raster-opacity': 1 // set < 1 if you want to blend with underlying style
    }
  });
}

// Now push ALL symbol/label layers back above the raster
// (raster is currently the top-most layer; this re-raises labels so they’re readable)
for (const l of map3d.getStyle().layers) {
  if (l.type === 'symbol') {
    // moveLayer with no "before" moves it to the top, above the raster
    map3d.moveLayer(l.id);
        ...
        // 3D buildings (helps sell 3D in urban areas)
        try {
          map3d.addLayer({
            id: '3d-buildings',
            source: 'composite',
      ...
      map3d.addLayer({ id:'sky', type:'sky', paint:{
        'sky-type':'atmosphere',
  ...
  // Try to coerce a segment's coordinates into [[lat,lon], ...]
  function normalizeCoords(seg) {
    ...
    for (const p of A) {
      ...
      // array [a,b] but unknown order
      if (Array.isArray(p) && p.length >= 2) {
        const a = +p[0], b = +p[1];
        if (!Number.isFinite(a) || !Number.isFinite(b)) continue;
        // heuristic: lat is within [-90,90], lon within [-180,180]
        ...
        if (looksLatLon && !looksLonLat) out.push([a, b]);
          ...
          // ambiguous: prefer lon,lat -> lat,lon swap if |a|>90 and |b|<=90
  ...
  function slopeToColour(min, max, val) {
    ...
    // --- helpers to build a quad around a line segment in geographic degrees ---
  ...
  function quadForSegment(c1, c2, halfWidthM) {
    ...
    // vector from c1 to c2 in meters
    const vx_m = (c2[1] - c1[1]) * mPerDegLon;
    const vy_m = (c2[0] - c1[0]) * mPerDegLat;
    const L = Math.hypot(vx_m, vy_m) || 1;
    // perpendicular unit (to the left)
    const nx = -vy_m / L, ny = vx_m / L;
    const dx_m = nx * halfWidthM, dy_m = ny * halfWidthM;
    // convert back to degrees
    const dLon = dx_m / mPerDegLon, dLat = dy_m / mPerDegLat;
    const dLonR = -dLon, dLatR = -dLat;
    // corners in [lon,lat] order for GeoJSON polygon
 ...
 // Open-Elevation has URL length limits; chunk requests (~100 pts per call)
  ...
  async function render3DSegment(seg) {
    ...
    for (let i = 0; i < slopes.length; i++) {
      ...
      const z1 = elevs[i],  z2 = elevs[i+1];
      const colour = slopeToColour(minSlope, maxSlope, slopes[i]);
      // line (for crisp outline)
      lineFeatures.push({
        type: 'Feature',
        geometry: { type: 'LineString', coordinates: [[c1[1], c1[0], z1], [c2[1], c2[0], z2]] },
        properties: { colour }
      });
      // skin quad (skip if decimating)
      if (i % SKIN_STRIDE === 0) {
    ...
    // add coloured line on top
    map3d.addSource('segment', { type:'geojson',
    ...
    const bounds = coords.reduce(
      ...
      new mapboxgl.LngLatBounds([coords[0][1], coords[0][0]], [coords[0][1], coords[0][0]])
    );
    map3d.fitBounds(bounds, { padding:24, maxZoom:17, duration:400 });
    map3d.once('idle', () => {
      map3d.resize();
      if (map3d.getPitch() < 45) map3d.setPitch(60, { duration: 0 });
      if (Math.abs(map3d.getBearing()) < 1) map3d.setBearing(-17.6, { duration: 0 });
    });
  }

  const prevWavelet = window.onWaveletResponse;
  window.onWaveletResponse = function (out) {
    if (typeof prevWavelet === 'function') prevWavelet(out);
    window.segmentsGlobal = (out && Array.isArray(out.segments)) ? out.segments : [];
    if (window.MAPBOX_TOKEN && window.segmentsGlobal.length) render3DSegment(window.segmentsGlobal[0]);
  };
