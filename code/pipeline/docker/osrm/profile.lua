...
function setup()
	...
	return {
		...
		bicycle_speeds = {
			trunk = default_speed * 2,
			cycleway = default_speed * 1.2,
			primary = default_speed * 2.4,
			primary_link = default_speed * 2.0,
			secondary = default_speed * 1.5,
			secondary_link = default_speed * 1.2,
			tertiary = default_speed * 2.0,
			tertiary_link = default_speed * 2.1,
			residential = default_speed * 1.1,
			unclassified = default_speed,
			living_street = default_speed,
			road = default_speed * 2.0,
		...
		surface_speeds = {
			asphalt = default_speed * 4,
			chipseal = default_speed * 3,
...
function handle_bicycle_tags(profile, way, result, data)
	...
	if
		(not data.highway or data.highway == "")
		and (not data.route or data.route == "")
		and (not profile.use_public_transport or not data.railway or data.railway == "")
		and (not data.amenity or data.amenity == "")
		and (not data.man_made or data.man_made == "")
		and (not data.public_transport or data.public_transport == "")
		and (not data.bridge or data.bridge == "")
	then
		return false
	end
...
function speed_handler(profile, way, result, data)
	...
	-- railways
	elseif
		profile.use_public_transport
		and data.railway
		and profile.railway_speeds[data.railway]
		and profile.access_tag_whitelist[data.access]
	then
...
function cycleway_handler(profile, way, result, data)
	...
	data.is_twoway = result.forward_mode ~= mode.inaccessible
		and result.backward_mode ~= mode.inaccessible
		and not data.implied_oneway
	...
	if data.is_twoway then
		...
		if
			(data.cycleway_right and profile.cycleway_tags[data.cycleway_right])
		...
		local has_twoway_cycleway = (data.cycleway and profile.opposite_cycleway_tags[data.cycleway])
			or (data.cycleway_right and profile.opposite_cycleway_tags[data.cycleway_right])
			or (data.cycleway_left and profile.opposite_cycleway_tags[data.cycleway_left])
		local has_opposite_cycleway = (data.cycleway_left and profile.opposite_cycleway_tags[data.cycleway_left])
			or (data.cycleway_right and profile.opposite_cycleway_tags[data.cycleway_right])
		local has_oneway_cycleway = (data.cycleway and profile.cycleway_tags[data.cycleway])
			or (data.cycleway_right and profile.cycleway_tags[data.cycleway_right])
			or (data.cycleway_left and profile.cycleway_tags[data.cycleway_left])
...
function bike_push_handler(profile, way, result, data)
	...
	if
		result.forward_mode == mode.inaccessible
		or result.backward_mode == mode.inaccessible
		or result.forward_speed == -1
		or result.backward_speed == -1
	then
